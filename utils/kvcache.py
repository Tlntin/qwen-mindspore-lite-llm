...
class KVCacheManger:
    def __init__(self, config: InferenceConfig) -> None:
        ...
        if self.session_type in ["onnx", "ms_lite"]:
            ...
            self.kv_cache = torch.zeros(
                self.past_key_value_shape, dtype=self.dtype, device=self.device_str
            )
        else:
            self.kv_cache = None
    ...
    def get_inputs(self, seq_len: int) -> List[np.ndarray]:
        ...
        """
        if self.session_type in ["onnx", "pytorch"]:
            temp_kv_size = self.real_kv_size + 1
        elif self.session_type == "ms_lite":
            temp_kv_size = self.past_kv_size
        else:
            raise Exception(f"unknow sesstion_type: {self.session_type}")
        cache = self.kv_cache[:, :, :temp_kv_size]
        # mask and pos_id all the [N, C, H, W] format
        if self.session_type in ["onnx", "ms_lite"]:
            mask = np.ones((1, 1, 1, temp_kv_size + seq_len),dtype=np.int64)
            ...
            mask = torch.ones((1, 1,  1, temp_kv_size + seq_len), dtype=torch.long, device=self.device_str)
            mask[:, :, :, self.real_kv_size: temp_kv_size] = 0
            pos_id =torch.arange(
                self.input_pos, 
                self.input_pos + seq_len,
            ...
            raise Exception(f"unknow session_type: {self.session_type}")
